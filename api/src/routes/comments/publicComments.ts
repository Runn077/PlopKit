...
router.post('/', commentBurstLimiter, commentHourlyLimiter, validate(createCommentSchema), async (req, res, next) => {
  try {
    const { widget_key, page_url, body, parent_id, quoted_id, author_name } = req.body
    const origin = req.headers.origin ?? req.headers.referer ?? ''
    const comment = await commentService.createComment(widget_key, page_url, body, parent_id, quoted_id, origin, author_name)
    res.json(comment)
  } catch (err) { next(err) }
})
