...
export function startScheduler() {
  // Clean up expired unverified sites daily at midnight
  cron.schedule('0 0 * * *', async () => {
    const deleted = await prisma.site.deleteMany({
      where: { verified: false, expiresAt: { lt: new Date() } },
    })
    console.log(`[scheduler] Cleaned up ${deleted.count} expired unverified sites`)
  })
